...
- **✅ Megoldva (2026.09.21): "Suspense vs main" hidratáció-mismatch.** A korábban itt leírt, alkalmankénti "Hydration failed... React error #418" hiba (`<Suspense>` vs. `<main>` diff, bármelyik route-on) gyökéroka nem timing race volt, hanem egy determinisztikus framework-mismatch: a `@tanstack/react-router` `<Matches>` komponense (lásd a csomag `Matches.tsx`-ét) csak akkor NEM tesz `<Suspense>`-et minden route-tartalom köré kliens oldalon, ha a router `ssr` property-je (nem a route-opció, a router-instance mező) igaz értékű — ezt normál esetben a keretrendszer saját `hydrate()` segédfüggvénye állítja be (`@tanstack/router-core/ssr/ssr-client.ts`) egy valódi TanStack Start SSR-bootnál. Ez a bypass-elt GH Pages pipeline soha nem hívja a `hydrate()`-et (`spa-entry.tsx` helyette simán `router.load()` + `hydrateRoot()`-ot csinál), így `router.ssr` sosem állt be, és a kliens **minden egyes** hidratációnál `<Suspense>`-be csomagolta a route tartalmát — miközben a szerver (`entry-prerender.tsx`, `renderToString()`) sosem, mert ott egy külön `isServer` ág rövidre zárja ugyanezt a döntést. Javítás: `src/spa-entry.tsx`-ben `router.ssr = { manifest: undefined }` beállítása `hydrateRoot()` előtt (lásd a fájlban lévő kommentet) — ez tükrözi a valódi `hydrate()` saját `router.ssr = { manifest }` sorát, a `window.$_TSR` dehidratációs adatfüggőség nélkül, amink nincs is. Bónuszként ez megszünteti a `Transitioner.tsx` által ugyanerre a flagre épített, hidratáláskor lefutó felesleges második `router.load()`-ot is. **Nem** ugyanaz, mint a `__root.tsx` `useRouterState`-es próbálkozása (lásd fentebb) — más komponens, más mechanizmus, és nem lett visszavonva. Ellenőrizve: `git stash`-es kontroll-build (a hiba előtti kód) determinisztikusan reprodukálta a hibát minden betöltésnél dev-mode unminified buildben; a javított kód 10/10 tiszta betöltés `/`-en, plusz tiszta `/cikkek/` és `/en/` dev-mode-ban, és tiszta `/`, `/cikkek/`, `/en/` egy sima `npm run build:ghpages` production buildben.
